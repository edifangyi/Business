...
public class AddGoods_4_Activity extends BaseActivity implements View.OnClickListener {
    ...
    @BindView(R.id.fy_FDCStock_1)
    FYBtnRadioView fyFDCStock1;
    @BindView(R.id.fy_FDCStock_2)
    FYBtnRadioView fyFDCStock2;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_goods_1);
    ...
    private void setinitItemView() {
        ...
        fyFDCStock1.setTitle("调入仓库");
        fyFDCStock1.setContent(mapWareHouse.get(fDCStockD));
        if ("0".equals(fStockPosition)) {
            fyFDCStock2.setVisibility(View.GONE);
        }

        fyFDCStock2.setTitle("调出仓库");
        fyFDCStock2.setContent(mapWareHouse.get(fDCStockD));
        if ("0".equals(fStockPosition)) {
            fyFDCStock2.setVisibility(View.GONE);
        }
    ...
    private void setListener() {
        ...
        fyFDCStock1.setOnClickListener(this);
        fyFDCStock2.setOnClickListener(this);
...
//                fyPrice.setSelection(AmountShow(Double.parseDouble(price)).length()); //光标置后
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.fy_Barcode://条形码
                break;
            case R.id.fy_FDCStock_1://仓库
                setFDCStock1();
                break;
            case R.id.fy_FDCStock_2://仓库
                setFDCStock2();
                break;
            case R.id.fy_Uom://单位
                setUom();
    ...
    private void setFDCStock1() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(AddGoods_4_Activity.this);
        builder.setTitle("仓库选择");
        builder.setSingleChoiceItems(arrWareHouse, intFDCStockNum1, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                intFDCStockNum1 = which;
                //2.设置
                fyFDCStock1.setContent(arrWareHouse[which]);
                dialog.dismiss();
            }
        });
    ...
    private void setUom() {
        ...
        price = String.valueOf(mul(tradePrice, conversion));
    ...
    private void setGoodsName() {
        Intent intent = new Intent(AddGoods_4_Activity.this, SearchActivity.class);
    ...
    private void setGoodsPage(Bundle bundle, DBBusiness business) {
        ...
        //商品信息
        aloneGoodsMessageList = business.getBaseGoodsMessageIncludePrice(bundle.getString("sysid"));
        ...
        for (Map<String, String> aloneGoodsMessage : aloneGoodsMessageList) {
            if ("1".equals(aloneGoodsMessage.get("Ubase"))) {
                ...
                tradePrice = String.valueOf(mul(aloneGoodsMessage.get("TradePrice"), conversion));
            }
        }
    ...
    private void setCallbackData() {
        ...
        setResult(-1, AddGoods_4_Activity.this.getIntent().putExtras(bundle));//执行回调事件;
        AddGoods_4_Activity.this.finish();
    }
    ...
    private void saveAddGoods() {
        ...
        for (Map.Entry<String, String> stringStringEntry : mapWareHouse.entrySet()) {
            if (stringStringEntry.getValue().equals(fyFDCStock1.getText())) {
                bundle.putString("WareHouseSysId1", stringStringEntry.getKey().toString()); //仓库id
            }

            if (stringStringEntry.getValue().equals(fyFDCStock2.getText())) {
                bundle.putString("WareHouseSysId2", stringStringEntry.getKey().toString()); //仓库id
            }
        }


        setResult(ADD_GOODS_REQ_CODE, AddGoods_4_Activity.this.getIntent().putExtras(bundle));//执行回调事件;
        AddGoods_4_Activity.this.finish();
