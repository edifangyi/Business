...
public class FEntry26Fragment extends BaseFragment {
    ...
    private String strFPlanFetchDay;//交货日期
    ...
    public FEntry26Fragment() {

    }


    public static FEntry26Fragment getInstance(String s) {
        FEntry26Fragment newFragment = new FEntry26Fragment();
    ...
    protected View getSuccessView() {
        ...
        CommonUtils.setSaleTitle(getActivity(), "销售订货单");
    ...
    private void readingData() {
        ...
        fEntry26Set = manager.getFEntry_26_Set(new String[]{LoginUser.userSysID, "销售订货单"});
        manager.closeDB();
    ...
    private void setinitItemView() {
        ...
        strFPlanFetchDay = getTimeNYR(Integer.parseInt(fEntry26Set.FPlanFetchDay));


        fyFPlanFetchDay.setContent(getTime(strFPlanFetchDay));
        if ("0".equals(fEntry26Set.FFetchDayV)) {
            fyFPlanFetchDay.setVisibility(View.GONE);
        }

        fyFAreaPS.setTitle("销售范围");
        fyFAreaPS.setContent(map.get(fEntry26Set.fAreaPSD));//默认
        ...
        fyFSaleStyle.setContent(map.get(fEntry26Set.FSaleStyleD));//默认
        ...
        fyFDept.setTitle("部门");
        fyFDept.setContent(mapDepartment.get(fEntry26Set.fDeptD));
        if ("0".equals(fEntry26Set.fDeptV)) {
            fyFDept.setVisibility(View.GONE);
        }

        fyFDCStock.setTitle("仓库");
        fyFDCStock.setContent(mapWareHouse.get(fEntry26Set.fDCStockD));
...
//        recyclerView1.setHasFixedSize(true);// 如果Item够简单，高度是确定的，打开FixSize将提高性能。
    ...
    private void setListener() {
        ...
        //允许选择日期
        if ("1".equals(fEntry26Set.fChooseDate)) {
    ...
    private void uploadOrder() {
        ...
        if ("上传失败".equals(result)) {
            ...
            if ("2".equals(fEntry26Set.fDelType)) {//上传后不可以修改
                setClickEvent(false);
            }
        }
    }
    ...
    private void setBtnPrint() {
        ...
        if ("1".equals(fEntry26Set.fDelType)) {//打印后不可以修改
            setClickEvent(false);
        }
    }
    ...
    private void saveSettings() {
        ...
        if ("0".equals(fEntry26Set.fUpdateType)) {//保存后不可以修改
            setClickEvent(false);
        }
    }
    ...
    private void save() {
        ...
        String DeliveryDate = getStrToDate(strFPlanFetchDay); //
        ...
        String FUpdateType = fEntry26Set.fUpdateType; //修改控制
        String FDelType = fEntry26Set.fDelType; //删除控制
        ...
        isSaveOrder = "1";
        CommonUtils.setSaleTitle(getActivity(), "销售订货（已保存）");
    ...
    private void setFDCStock() {
        ...
        if (!"1".equals(fEntry26Set.fCheckType)) {
    ...
    private void setFAreaPS() {
        ...
        if (!fEntry26Set.fCheckType.equals("1")) {
            //滑动
    ...
    private String printOrder() {
        ...
        manager.closeDB();
        int pageSize = Integer.valueOf(fEntry26Set.fPrintSame);
        switch (pageSize) {
            case 0:
