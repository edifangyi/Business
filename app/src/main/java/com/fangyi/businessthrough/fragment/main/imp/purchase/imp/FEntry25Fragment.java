...
public class FEntry25Fragment extends BaseFragment {
    ...
    private FEntry_25_Set fEntry25Set;
    ...
    public FEntry25Fragment() {

    }

    public static FEntry25Fragment getInstance(String s) {
        FEntry25Fragment newFragment = new FEntry25Fragment();
    ...
    protected View getSuccessView() {
        ...
        CommonUtils.setPurchaseTitle(getActivity(), "采购退货");
    ...
    private void readingData() {
        ...
        fEntry25Set = manager.getFEntry_25_Set(new String[]{LoginUser.userSysID, "采购退货单"});
        manager.closeDB();
    ...
    private void setinitItemView() {
        ...
        fyFPOStyle.setContent(map.get(fEntry25Set.fPOStyleD));
        fyFPOStyle.setContentTextColor(getColor(R.color.text_user_account));
        if ("0".equals(fEntry25Set.fPOStyleV)) {
        ...
        fyFSupplier.setContentTextColor(getColor(R.color.text_user_account));

        fyFRequester.setTitle("业务员");
        fyFRequester.setContent(LoginUsers.kISName);
        fyFRequester.setContentTextColor(getColor(R.color.text_user_account));


        fyFDept.setTitle("部门");
        fyFDept.setContent(mapDepartment.get(fEntry25Set.fDeptD));
        fyFDept.setContentTextColor(getColor(R.color.text_user_account));
        if ("0".equals(fEntry25Set.fDeptV)) {
        ...
        fyFFManager.setContent(mapRequester.get(fEntry25Set.fFManagerD));
        fyFFManager.setContentTextColor(getColor(R.color.text_user_account));

        if ("0".equals(fEntry25Set.fFManagerV)) {
            fyFDept.setVisibility(View.GONE);
        }


        fyFSManager.setTitle("保管");
        fyFSManager.setContentTextColor(getColor(R.color.text_user_account));

        fyFSManager.setContent(mapRequester.get(fEntry25Set.fSManagerD));
        if ("0".equals(fEntry25Set.fSManagerV)) {
        ...
        fyFDCStock.setContent(mapWareHouse.get(fEntry25Set.fDCStockD));
        if ("0".equals(fEntry25Set.fDCStockV)) {
            fyFDCStock.setVisibility(View.GONE);
        }

        if ("1".equals(fEntry25Set.fStockPosition)) {//仓库位置在添加商品页面
            fyFDCStock.setVisibility(View.GONE);
        }
        ...
        if ("0".equals(fEntry25Set.fSaFePrint)) {
        ...
        if ("2".equals(fEntry25Set.fOnType)) {//批量
    ...
    private OnItemClickListener onItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(final int position) {
            ...
            if ("1".equals(fEntry25Set.fChooseAmount)) {//可以改价格
                fyPrice.setTitle("单价");
    ...
    private void over(int position) {
        ...
        if ("1".equals(fEntry25Set.fChooseAmount)) {//更改单价金额
    ...
    private void setListener() {
        if ("1".equals(fEntry25Set.fChooseDate)) {
    ...
    private void uploadOrder() {
        ...
        if ("上传失败".equals(result)) {
            ...
            if ("2".equals(fEntry25Set.fDelType)) {//上传后不可以修改
                setClickEvent(false);
            }
        }
    }
    ...
    private void setBtnPrint() {
        ...
        if ("1".equals(fEntry25Set.fDelType)) {//打印后不可以修改
            setClickEvent(false);
        }
    ...
    private void saveSettings() {
        ...
        KLog.e("==========" + fEntry25Set.fUpdateType);

        if ("0".equals(fEntry25Set.fUpdateType)) {//保存后不可以修改
            setClickEvent(false);
        }
    }
    ...
    private void save() {
        ...
        String strFUpdateType = fEntry25Set.fUpdateType;
        String strFDelType = fEntry25Set.fDelType;
    ...
    private void startAddGoods() {
        if (fyFSupplierID != null) {
            ...
            intent.putExtra("fChooseAmount", fEntry25Set.fChooseAmount);//允许修改单价金额
            intent.putExtra("fDCStockD", fEntry25Set.fDCStockD);//仓库
            intent.putExtra("fStockPosition", fEntry25Set.fStockPosition);//仓库 在添加页面
    ...
    private void setFDCStock() {
        ...
        if (!"1".equals(fEntry25Set.fCheckType)) {
    ...
    private void setFFManager() {
        ...
        if (!fEntry25Set.fCheckType.equals("1")) {
            //滑动
    ...
    private void setFPOStyle() {
        final String items[] = {map.get("20300"), map.get("252"), map.get("251")};
    ...
    private String printOrder() {
        ...
        manager.closeDB();
        int pageSize = Integer.valueOf(fEntry25Set.fPrintSame);
        switch (pageSize) {
            case 0:
