...
public class CharacterParser {
    ...
    private int getChsAscii(String chs) {
        int asc = 0;
        try {
            byte[] bytes = chs.getBytes("gb2312");//转换编码;
//			Log.e("55555555555555555555555555555555555555555555555555", msg);
            if (bytes == null || bytes.length > 2 || bytes.length <= 0) {//错误;
                throw new RuntimeException("illegal resource string");//输出错误;
            }
    ...
    public String convert(String str) {
        String result = null;
        int ascii = getChsAscii(str);//获取这个字符串的ASCII码;
        if (ascii > 0 && ascii < 160) {//不是汉字的话
            result = String.valueOf((char) ascii);
        } else {//判断是否为汉字;是汉字的话对照两个表进行拼音选择;
    ...
    //<------------------------------------------------------------------------->解析数组，拆分成一个一个，逐一进行判断返回的是拼音;
    /** * 词组解析 * * @param chs * @return */
    public String getSelling(String chs) {
        String key, value;
        buffer = new StringBuilder();
        for (int i = 0; i < chs.length(); i++) {
            key = chs.substring(i, i + 1);//chs.substring(i,i+1)这是一个String的截取字符串的方法，从第i个开始到i+1个。
            if (key.getBytes().length >= 2) {//返回一个字节数组;
